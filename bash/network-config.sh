...
mylanipaddress=$(ip a s $myinterfacename | awk '/inet /{gsub(/\/.*/,"");print $2}')
mylanhostname=$(getent hosts $mylanipaddress | awk '{print $2}')
myexternalip=$(curl -s icanhazip.com)
myexternalname=$(getent hosts $myexternalip | awk '{print $2}')
myrouteraddress=$(route -n | grep 'UG[ \t]' | awk '{print $2}')
myrouterhostname=$(route | grep "default" | awk '{print $2}')

cat <<EOF
Hostname        : $myhostname
LAN Address     : $mylanipaddress
LAN Hostname    : $mylanhostname
External IP     : $myexternalip
External Name   : $myexternalname
Router Address  : $myrouteraddress
Router Hostname : $myrouterhostname
EOF
